...
Task 4 人工验收时发现三个传输相关缺陷。前两个非本次改动引入；第三个的**根因早已存在，
但本次重构确实把它放大了**，详见下方说明。已另行安排修复：
...
- **`SSHExecutor` 单例并发冲突（根因）** —— 它是 `static let shared`，只有一个
  ...
  **本次重构放大了这个缺陷**：改动前传输用 `childPid`、`ssh` 目录列举用
  `runningProcess`，是两个独立字段，列举永远不会覆盖传输的句柄；改动后二者共用
  `runningProcess`，于是传输期间任何一次 `ssh ls`（发送面板、接收面板、覆盖检查、
  大小探测都会触发）都会把句柄改写成列举进程并在其结束时置 nil，让进行中的传输
  彻底无法取消。这是本次改动引入的退化，修复方案已包含它。
